...
class TestAugumentations(object):
    ...
    def assert_img_equal(img, ref_img, ratio_thr=0.999):
        """Approximatively match two imgs."""
    ...
    @staticmethod
    def check_keys(result_keys, target_keys):
        """Check created/updated keys exist."""
        for target_key in target_keys:
            if target_key not in result_keys:
                return False
        return True

    @staticmethod
    def check_crop(result_img_shape, result_bbox):
        """Check the cropped image has the same shape as `img_shape`."""
    ...
    def check_scale(origin_shape, target_shape, scale_factors, keep_ratio):
        """Check the scaled image in class `Resize` output."""
        if keep_ratio:
    ...
    def check_flip(origin_imgs, result_imgs, flip_type):
        """Check the flipped image in class `Flip` output."""
    ...
    def check_normalize(self, origin_imgs, result_imgs, norm_cfg):
        """Check the normalized image in class `Normalize` output."""
    ...
    def test_multi_scale_crop(self):
        ...
        assert self.check_keys(multi_scale_crop_results.keys(), target_keys)
    ...
    def test_resize(self):
        ...
        assert self.check_keys(resize_results.keys(), target_keys)
    ...
    def test_flip(self):
        ...
        assert self.check_keys(flip_results.keys(), target_keys)
    ...
    def test_normalize(self):
        ...
        assert self.check_keys(normalize_results.keys(), target_keys)
    ...
    def test_center_crop(self):
        ...
        assert self.check_keys(center_crop_results.keys(), target_keys)
    ...
    def test_three_crop(self):
        ...
        assert self.check_keys(three_crop_results.keys(), target_keys)
    ...
    def test_ten_crop(self):
        ...
        assert self.check_keys(ten_crop_results.keys(), target_keys)
